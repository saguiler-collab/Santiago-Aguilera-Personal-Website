# santiaguilera - Personal Website:

**I am building a personal website for universities to check out and understand my entire life**

**App should**:
  Allow playing videos for other people
  Implement uploaded pictures
  Have a proper and very well-made design
  Be interactive in a way, add sliders about each city showing pictures of each one, or have scroll animations and numbers that count up     to show stuff about me.
  *Turn it into an interactive life map/timeline.*
  
Sections, subsections, different pages:

Homepage
Who am I?
  Mexico (Monterrey, NL)
  Colombia (Bogota)
  Panama (Ciudad de Panama)
  Trinidad & Tobago (Port of Spain)
  United States (Pennsburg, PA)
Academics - Each main page will be academic achievements
  International School of Port of Spain (ISPS)
    StuCo
    Stronger Together
    
    
  Perkiomen School
    HOSA Club
    Investment Club
    Perkiomenite Org
    GVMUN - Global Virtual Model United Nations
    Model United Nations
    Science Fair + Olympiad (Not prepared so don't build yet)
    
Athletics
  Weightlifting + Olympic Lifting
  Varsity Swimming
Drum Playing
  SonoArte Panama
  ISPS Musical Playing
  Perkiomen Rock Band Playing
Hobbies
  Speaking French + Chinese
  Reading Manga
  Playing Videogames

Bottom (Icons):
Instagram Link
LinkedIn Link
Discord Link

**Secondary Plan:**

HOME
│
├── About Me
│   ├── My Story
│   ├── Places I've Lived
│   └── What I Care About
│
├── Academics
│   ├── Academic Journey
│   ├── Research
│   ├── HOSA
│   ├── MUN + GVMUN
│   ├── Journalism / Perkiomenite
│   └── Science & Olympiad [Coming Soon]
│
├── Research & Projects
│   ├── Current Research
│   ├── Publications
│   ├── Medical App / Hospital Project
│   └── Future Research
│
├── Leadership & Service
│   ├── HOSA
│   ├── GVMUN
│   ├── Hospital Shadowing
│   └── Community Service Club / Volunteering
│
├── Beyond the Classroom
│   ├── Swimming
│   ├── Weightlifting / Olympic Lifting
│   ├── Drums
│   └── Music
│
├── Curiosities
│   ├── Languages
│   ├── Books & Manga
│   ├── Music / Spotify
│   └── Video Games
│
└── Media
    ├── Photos
    └── Videos
